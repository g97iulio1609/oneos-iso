...
make_packages() {
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${pacman_conf}" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/packages.{both,${arch}}) --force " install
}
...
make_customize_root-image() {
    ...
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${pacman_conf}" -D "${install_dir}" -r '/root/customize_root-image.sh' run
    rm ${work_dir}/${arch}/root-image/root/customize_root-image.sh
}
...
# Build root-image filesystem image
make_prepare() {
...
for arch in x86_64; do
    ...
    run_once make_customize_root-image
done
